...
class DataModeler:
    ...
    def prepare_data(self, oos_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        '''
        # ** Your code here **
        # prepare data on the training set
        if oos_df is None:
            self.train_labels_df = self.train_df.loc[:,['outcome']].copy()
            self.train_df = self.train_df.loc[:,['amount','transaction_date']].copy()
            self.train_df['transaction_date'] = self.train_df.transaction_date.apply(lambda x: datetime.strptime(x, '%Y-%m-%d').replace(tzinfo=timezone.utc).timestamp()*10**9 if x is not None else None)
        # prepare data on the external data set
        else:
            oos_df = oos_df.loc[:,['amount','transaction_date']].copy()
            oos_df['transaction_date'] = oos_df.transaction_date.apply(lambda x: datetime.strptime(x, '%Y-%m-%d').replace(tzinfo=timezone.utc).timestamp()*10**9 if x is not None else None)
            return oos_df
    ...
    def impute_missing(self, oos_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        '''
        # ** Your code here **
        # impute data on the training set
        if oos_df is None:
            self.train_mean_values = self.train_df.mean()
            self.train_df.fillna(self.train_mean_values,inplace=True)
        # impute data on the external data set
        else:
            oos_df.fillna(self.train_mean_values,inplace=True)
            return oos_df
